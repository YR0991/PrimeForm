...
/** Minimal view: event labels from sourceSummary (Check-in saved, Baseline import, Strava sync). */
function eventSummary(row) {
  const s = row.sourceSummary || {}
  const parts = []
  if (s.hasCheckin === true) parts.push('Check-in saved')
  if (s.hasImport === true) parts.push('Baseline import')
